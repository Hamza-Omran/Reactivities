...
public class ProfilesController : BaseApiController
{
    ...
    [HttpPost("add-photo")]
    // as we specified only one single parameter then we don't need to write [FromForm]
    public async Task<ActionResult<Photo>> AddPhoto(IFormFile file)
    {
        return HandleResults(await Mediator.Send(new AddPhoto.Command{File = file})); 
    }
    
    [HttpGet("{userId}/photos")]
    public async Task<ActionResult<List<Photo>>> GetPhotosForUser(string userId)
    {
        return HandleResults(await Mediator.Send(new GetProfilePhotos.Query{UserId = userId})); 
    }
    
    [HttpDelete("{photoId}/photos")]
    public async Task<ActionResult> DeletePhoto(string photoId)
    {
        return HandleResults(await Mediator.Send(new DeletePhoto.Command{PhotoId = photoId})); 
    }
    
    [HttpPut("{photoId}/setMain")]
    public async Task<ActionResult> SetMainPhoto(string photoId)
    {
        return HandleResults(await Mediator.Send(new SetMainPhoto.Command{PhotoId = photoId})); 
    }

    [HttpGet("{userId}")]
    public async Task<ActionResult<UserProfile>> GetProfile(string userId)
    {
        return HandleResults(await Mediator.Send(new GetProfile.Query{UserId = userId})); 
    }
    ...
    {
        return HandleResults(await Mediator.Send(new GetUserActivities.Query{UserId = userId, Filter = filter}));
    }
}
